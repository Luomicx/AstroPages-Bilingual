...
> 
> 【黑马程序员 MySQL 数据库入门到精通，从 mysql 安装到 mysql 高级、mysql 优化全囊括】 [https://www.bilibili.com/video/BV1Kr4y1i7ru](https://www.bilibili.com/video/BV1Kr4y1i7ru)
...
更多权限请看[权限一览表](https://jimhackking.github.io/%E8%BF%90%E7%BB%B4/MySQL%E5%AD%A6%E4%B9%A0%E7%AC%94%E8%AE%B0/#%E6%9D%83%E9%99%90%E4%B8%80%E8%A7%88%E8%A1%A8)
...
> 这三个问题的详细演示：[https://www.bilibili.com/video/BV1Kr4y1i7ru?p=55cd](https://www.bilibili.com/video/BV1Kr4y1i7ru?p=55cd)
...
![image-20250720233738136](http://img.wiretender.top/img/20250720233738280.webp)
...
![image-20250720230330428](http://img.wiretender.top/img/20250720230330619.webp)
...
![image-20250720230627951](http://img.wiretender.top/img/20250720230628090.webp)
...
![image-20250720233429074](http://img.wiretender.top/img/20250720233429244.webp)
...
![image-20250720233807340](http://img.wiretender.top/img/20250720233807469.webp)

- InnoDB， MyISAM， Memory 存储引擎都支持 B+索引，一般考察索引都是 **B+树索引**

![image-20250720234205568](http://img.wiretender.top/img/20250720234205700.webp)
...
![image-20250720234835748](http://img.wiretender.top/img/20250720234835889.webp)

### B+树索引

![image-20250720234857624](http://img.wiretender.top/img/20250720234857778.webp)

### 哈希索引

![image-20250720235102741](http://img.wiretender.top/img/20250720235102903.webp)

- **Q2：**

![image-20250720235408595](http://img.wiretender.top/img/20250720235408729.webp)
...
![image-20250721000015608](http://img.wiretender.top/img/20250721000015777.webp)
...
![image-20250721221617432](http://img.wiretender.top/img/20250721221618162.webp)
...
1. 副作用
   · 更多 IO：父层指针+新页+双页写入；
   · 空间碎片：页 8 KB 数据+8 KB 空洞，SHOW TABLE STATUS 看见 *Data\_free* 上升。
...
1. 避免&治理
   ...
   · 监控 `information_schema.innodb_metrics` 里的 **merge\_failure/split** 两指标判断是否需要调参。
...
![image-20250722141258301](http://img.wiretender.top/img/20250722141258749.webp)
...
1. LEAVE: 配合循环使用，退出循环。
1. ITERATE: 必须用在循环中，作用是跳过当前循环剩下的语句，直接进入下一次循环。
...
![image-20250722152144921](http://img.wiretender.top/img/20250722152145245.webp)
...
1. 如果在从库上备份，那么在备份期间从库不能执行主库同步过来的二进制日志(binlog)，会导致主从延迟。（该结构会在后续主从复制讲解）
...
- SHARED_READ
- 与 SHARED_READ、SHARED_WRITE 兼容，与 EXCLUSIVE 互斥
---
...
1. 间隙锁(GapLock): 锁定索引记录间隙(不含该记录)，确保索引记录间隙不变，防止其他事务在这个间隙进行 insert，产生幻读。在 RR 隔离级别下都支持。
1. 临键锁(Next-Key Lock): 行锁和间隙锁组合，同时锁住数据，并锁住数据前面的间隙 Gap。在 RR 隔离级别下支持。

![image-20250722155529114](http://img.wiretender.top/img/20250722155529427.webp)
...
1. 排他锁(X)：允许获取排他锁的事务更新数据，阻止其他事务获得相同数据集的共享锁和排他锁。
...
- 共享锁
- 需要手动 select 之后加上 lock in share mode
---
- select … for update
- 排他锁
- 需要手动在 select 之后 for update
{% /table %}

默认情况下，InnoDB 在 REPEATABLE READ 事务隔离级别运行，InnoDB 使用 next-key 锁进行搜索和索引扫描，以防止幻读。

1. 针对唯一索引进行检索时，对已存在的记录进行等值匹配时，将会自动优化为行锁。
1. InnoDB 的行锁是针对于索引加的锁，不通过索引条件检索数据，那么! nnoDB 将对表中的所有记录加锁，此时 **就会升级为表锁**。
...
![image-20250722155159455](http://img.wiretender.top/img/20250722155159768.webp)

#### Next-Key Lock（临键锁）

![image-20250722155217292](http://img.wiretender.top/img/20250722155217592.webp)

默认情况下，InnoDB 在 REPEATABLE READ 事务隔离级别运行，InnoDB 使用 next-key 锁进行搜索和索引扫描，以防止幻读。

1. 索引上的等值查询(唯一索引)，给不存在的记录加锁时, 优化为间隙锁 。
1. 索引上的等值查询(普通索引)，向右遍历时最后一个值不满足查询需求时，next-keylock 退化为间隙锁。
...
![image-20250724224108984](http://img.wiretender.top/img/20250724224109287.webp)

### 逻辑存储架构

![image-20250722230522899](http://img.wiretender.top/img/20250722230523313.webp)
...
![image-20250722231007169](http://img.wiretender.top/img/20250722231007445.webp)
...
![image-20250724223819539](http://img.wiretender.top/img/20250724223819819.webp)
...
![image-20250724222535859](http://img.wiretender.top/img/20250724222536630.webp)
...
![image-20250724223532164](http://img.wiretender.top/img/20250724223532484.webp)

## MySQL 管理

![image-20250724225231577](http://img.wiretender.top/img/20250724225231885.webp)
...
---
- mysql
- 存储MVSQL服务器正常运行所需要的各种信息(时区、主从、用户、权限等)
---
- information_schema
...
---
- sys
- 包含了一系列方便 DBA和开发人员利用 performance_schema性能数据库进行性能调优和诊断的视图
{% /table %}
...
![image-20250724224528744](http://img.wiretender.top/img/20250724224529028.webp)

#### mysqladmin

![image-20250724224638523](http://img.wiretender.top/img/20250724224638844.webp)

#### mysqlbinlog

![image-20250724224855039](http://img.wiretender.top/img/20250724224855309.webp)

#### mysqlshow

![image-20250724225040480](http://img.wiretender.top/img/20250724225040780.webp)

#### mysqldump

![image-20250724225106737](http://img.wiretender.top/img/20250724225107152.webp)

#### mysqlimport/source

![image-20250724225142593](http://img.wiretender.top/img/20250724225142942.webp)
...
![image-20250721000331286](http://img.wiretender.top/img/20250721000331378.webp)
...
- 高度为 2： n *8 + (n + 1)* 6 = 16 * 1024, 算出 n 约为 1170 这个节点下边最多会有 1171 个指针
...
![image-20250724230751332](http://img.wiretender.top/img/20250724230751637.webp)

### 原理

在MySQL中主要使用了binlog日志文件来实现主从复制。

![image-20250724231047085](http://img.wiretender.top/img/20250724231047460.webp)

### 搭建实现

![image-20250724231223800](http://img.wiretender.top/img/20250724231224118.webp)

#### 主库配置

![image-20250724231353770](http://img.wiretender.top/img/20250724231354020.webp)

![image-20250724231851608](http://img.wiretender.top/img/20250724231851906.webp)

#### 从库配置

![image-20250724232029816](http://img.wiretender.top/img/20250724232030163.webp)

![image-20250724232059351](http://img.wiretender.top/img/20250724232059745.webp)

![image-20250724232212972](http://img.wiretender.top/img/20250724232213438.webp)
...
![image-20250725134939634](http://img.wiretender.top/img/20250725134939980.webp)

#### 垂直拆分

![image-20250725135124374](http://img.wiretender.top/img/20250725135124607.webp)
...
![image-20250725135203002](http://img.wiretender.top/img/20250725135203228.webp)
...
![image-20250725135424288](http://img.wiretender.top/img/20250725135424533.webp)
...
![image-20250725135431231](http://img.wiretender.top/img/20250725135431460.webp)
...
● shardingJDBC:基于AOP原理，在应用程序中对本地执行的SQL进行拦截，解析、改写、路由处理。需要自行编码配置实现，只支持
java语言，性能较高。
...
Mycat是开源的、活跃的、基与Java语言编写的MySOL数据库中间件。可以像使用mysql一样来使用mycat,对于开发人员来说根本感觉
不到mycat的存在。
...
![image-20250725140412506](http://img.wiretender.top/img/20250725140412847.webp)

#### 入门

![image-20250725141150594](http://img.wiretender.top/img/20250725141150843.webp)

![image-20250725141807814](http://img.wiretender.top/img/20250725141808068.webp)

![image-20250725141825257](http://img.wiretender.top/img/20250725141825561.webp)

#### Mycat配置

![image-20250725142610816](http://img.wiretender.top/img/20250725142611056.webp)

![image-20250725142754900](http://img.wiretender.top/img/20250725142755180.webp)

![image-20250725142833662](http://img.wiretender.top/img/20250725142833939.webp)

![image-20250725142939182](http://img.wiretender.top/img/20250725142939451.webp)

![image-20250725142949826](http://img.wiretender.top/img/20250725142950107.webp)

**Mycat可以用到再详细理解，这里就烂尾咯\~\~**

## 读写分离

> 这里引用Javaguide中的文章内容 [读写分离](https://javaguide.cn/high-performance/read-and-write-separation-and-library-subtable.html)

# 完结

这里我们全部的内容就学习完成了，关于后边的运维篇，涉及到我们面试需要的可以留到看八股的时候再看，所以这里的笔记有些水，需要更详细的关于MySQL的讲解可以参考以下网站：

- [小林Coding](https://xiaolincoding.com/mysql/)
- [JavaGuide面试指导](https://javaguide.cn/)
